...
const EditInvoice = () => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                if (invoiceRes.data.success && invoiceRes.data.data) {
                    ...
                    const safeProducts = Array.isArray(productsRes.data.data) ? productsRes.data.data : [];

                    const populatedProducts = fetchedInvoice.products.map(item => {
                        const productDetails = safeProducts.find(p => p._id === item.product);
                        return {
                            ...item,
                    ...
                    setInvoiceData({
                        ...
                        supplier: fetchedInvoice.supplier?._id || fetchedInvoice.supplier || "", // Беремо ID постачальника
                        invoiceNumber: fetchedInvoice.invoiceNumber || "", // Номер накладної
                    });
                } else {
                    toast.error(invoiceRes.data.message || "Помилка завантаження накладної");
                    setInvoiceData(null);
                }
    ...
    const filteredProductsOptions = useMemo(() => {
        if (!invoiceData?.supplier || suppliers.length === 0 || products.length === 0) return [];
        ...
        return products
            .filter(product => product.category && allowedCategories.includes(product.category))
            .map(product => ({
                value: product._id,
    ...
    const handleAddProductToList = () => {
        if (!productToAdd?.value) { toast.error("Оберіть товар"); return; }
        if (!sizeToAdd) { toast.error("Оберіть розмір"); return; }
        const quantityNum = parseInt(quantityToAdd, 10);
        if (isNaN(quantityNum) || quantityNum <= 0) { toast.error("Введіть коректну кількість (>0)"); return; }
        if (pricePerUnitToAdd < 0) { toast.warn("Ціна за одиницю не може бути від'ємною."); } // Додав перевірку на від'ємну ціну

        const productDetails = products.find(p => p._id === productToAdd.value);
        if (!productDetails) { toast.error("Помилка: Деталі товару не знайдено"); return; }

        const existingItemIndex = invoiceData.products.findIndex(item =>
            item.product === productToAdd.value && item.size === sizeToAdd
        );
        ...
        if (existingItemIndex > -1) {
            ...
            toast.info(`Кількість товару "${productDetails.name} (${sizeToAdd})" оновлено.`);
        } else {
            const newItem = {
    ...
    const handleRemoveProductFromList = (indexToRemove) => {
        const updatedItems = invoiceData.products.filter((_, index) => index !== indexToRemove);
        setInvoiceData(prev => ({ ...prev, products: updatedItems }));
        recalculateTotalAmount(updatedItems);
    };
    ...
    const onSubmitHandler = async (event) => {
        ...
        try {
            ...
            const response = await axios.post(`${url}/api/invoices/edit-invoice`, dataToSend);
    ...
    // --- Рендеринг ---
    if (loading) { /* ... стан завантаження ... */ }
    if (!invoiceData) { /* ... стан помилки ... */ }

    const supplierName = suppliers.find(s => s._id === invoiceData.supplier)?.companyName || "Невідомий постачальник";
                ...
                <form onSubmit={onSubmitHandler} className="space-y-6">
                    ...
                    <fieldset className="border border-gray-300 p-4 rounded-md">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 items-start">
                            ...
                            <div className="flex flex-col gap-y-1 lg:col-span-2">
                                <label htmlFor="product-select" className='text-sm font-medium text-gray-600'>Товар</label>
                                <Select inputId="product-select" options={filteredProductsOptions} value={productToAdd} onChange={handleProductToAddChange} placeholder="Пошук товару..." isClearable noOptionsMessage={() => "Немає доступних товарів"} styles={{ control: (base) => ({ ...base, minHeight: '38px' }), menu: base => ({ ...base, zIndex: 5 }) }} theme={(theme) => ({ ...theme, borderRadius: 6, colors: { ...theme.colors, primary25: '#e0f2fe', primary: '#3b82f6' } })} />
                            </div>
                            {/* Розмір */}
                            <div className="flex flex-col gap-y-1">
                                <label htmlFor="size-add-select" className='text-sm font-medium text-gray-600'>Розмір</label>
                                <select id="size-add-select" value={sizeToAdd} onChange={handleSizeToAddChange} className="border border-gray-300 rounded-md py-1.5 px-3 outline-none focus:ring-1 focus:ring-offset-1 focus:ring-blue-500 focus:border-blue-500 h-[38px] transition duration-150 ease-in-out bg-white text-sm" disabled={!productToAdd || availableSizesForSelectedProduct.length === 0}>
                                    <option value="" disabled>-- Розмір --</option>
                                    {availableSizesForSelectedProduct.map((sizeInfo) => (<option key={sizeInfo.size} value={sizeInfo.size}>{sizeInfo.size}</option>))}
                                </select>
                            </div>
                            {/* Кількість */}
                            <div className="flex flex-col gap-y-1">
                                <label htmlFor="quantity-add-input" className='text-sm font-medium text-gray-600'>Кількість</label>
                                <input id="quantity-add-input" type="number" value={quantityToAdd} onChange={handleQuantityToAddChange} onBlur={handleQuantityToAddBlur} min="1" placeholder="1" className="border border-gray-300 rounded-md py-1.5 px-3 outline-none focus:ring-1 focus:ring-offset-1 focus:ring-blue-500 focus:border-blue-500 h-[38px] transition duration-150 ease-in-out text-sm w-full" disabled={!sizeToAdd} />
                            </div>
                            {/* Кнопка Додати */}
                            <div className="self-end pb-1">
                                <button type="button" onClick={handleAddProductToList} className="w-full inline-flex items-center justify-center gap-x-2 px-4 py-2 bg-[#0a6e13] text-white font-medium rounded-lg shadow-sm hover:bg-[#08580f] focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-[#0a6e13] transition text-sm h-[38px] disabled:opacity-50 disabled:cursor-not-allowed" disabled={!sizeToAdd || !productToAdd} >
                                    <FaPlus /> Додати
                                </button>
                            </div>
                            {/* Ціна/од. */}
                    ...
                    <fieldset className="border border-gray-300 p-4 rounded-md">
                        ...
                        {invoiceData.products.length > 0 ? (
                                    ...
                                    <tbody>
                                        {invoiceData.products.map((item, index) => (
                                            <tr key={`${item.product}-${item.size}-${index}`}>
                                                <td className="p-2 border align-middle">{item.name || "Невідомий товар"}</td>
                                                <td className="p-2 border text-center align-middle">{item.size}</td>
                                                {/* --- Редагована Кількість --- */}
                                                <td className="p-1 border text-center align-middle"> {/* Зменшив p-1 */}
                                                    <div className='flex items-center justify-center gap-1'>
                                                        {/* Кнопка мінус */}
                                                        <button
                                                            type='button'
                                                            onClick={() => handleQuantityChangeInList(index, String(item.quantity - 1))}
                                                        ...
                                                        <input
                                                            ...
                                                            onBlur={(e) => handleQuantityBlurInList(index, e.target.value)} // Додав onBlur
                                                            min="1"
                                                ...
                                                <td className="p-2 border text-right align-middle">{item.pricePerUnit.toFixed(2)} грн</td>
                                                <td className="p-2 border text-right align-middle font-medium">{(item.quantity * item.pricePerUnit).toFixed(2)} грн</td>
                            ...
                            <p className="text-gray-500 italic p-4 text-center">Додайте товари до накладної.</p>
                        )}
                    </fieldset>

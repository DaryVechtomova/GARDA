        ...
        const fetchCurrentUser = async () => {
            ...
            try {
                ...
                if (response.data.success && response.data.userData) {
                    ...
                    toast.error(response.userData.message || "Не вдалося завантажити дані профілю");
    ...
    const handleProfileSave = async (e) => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                localStorage.setItem("adminUserData", JSON.stringify(response.data.updatedUser));
            } else {
                    ...
                    <form onSubmit={handleProfileSave} className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                            ...
                            <div className="flex flex-col gap-y-1">
                                ...
                                <input id="firstName" name="firstName" type="text" value={userData.firstName} onChange={onChangeHandler} className="border border-gray-300 rounded-md py-1.5 px-3 h-[38px] outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500" />
                            </div>
                            {/* Прізвище */}
                            <div className="flex flex-col gap-y-1">
                                <label htmlFor="secondName" className='text-sm font-medium text-gray-600'>Прізвище <span className="text-red-500">*</span></label>
                                <input id="secondName" name="secondName" type="text" value={userData.secondName} onChange={onChangeHandler} className="border border-gray-300 rounded-md py-1.5 px-3 h-[38px] outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500" />
                            </div>
                            {/* По батькові */}
                            <div className="flex flex-col gap-y-1">
                                <label htmlFor="middleName" className='text-sm font-medium text-gray-600'>По батькові <span className="text-red-500">*</span></label>
                                <input id="middleName" name="middleName" type="text" value={userData.middleName} onChange={onChangeHandler} className="border border-gray-300 rounded-md py-1.5 px-3 h-[38px] outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500" />
                            </div>
                            {/* Телефон */}
                            <div className="flex flex-col gap-y-1">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 pt-4 border-t mt-4">
                            ...
                            <div className="flex flex-col gap-y-1">
                                <label htmlFor="hireDate" className='text-sm font-medium text-gray-600'>Дата прийому на роботу <span className="text-red-500">*</span></label>
                                <input
                                    id="hireDate"
                                    name="hireDate"
                                    type="date"
                            ...
                            <div className="flex flex-col gap-y-1">
                                <label className='text-sm font-medium text-gray-500'>Email (Логін) </label>
                                <div className="border border-gray-200 bg-gray-100 rounded-md py-1.5 px-3 h-[38px] flex items-center text-gray-700">{userData.email}</div>
                            </div>
                            <div className="flex flex-col gap-y-1">
                                <label className='text-sm font-medium text-gray-500'>Роль</label>
                                <div className="border border-gray-200 bg-gray-100 rounded-md py-1.5 px-3 h-[38px] flex items-center text-gray-700 capitalize">{userData.role}</div>
                            </div>
                        </div>

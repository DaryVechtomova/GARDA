...
const SupplierDetails = () => {
    ...
    const getStatusStyle = (status) => {
        switch (status?.toLowerCase()) { // Додав ?.toLowerCase() для безпеки
            case "активний": return "bg-green-100 text-green-800";
            case "на розгляді": return "bg-yellow-100 text-yellow-800";
            case "призупинений": return "bg-orange-100 text-orange-800";
            case "завершений": return "bg-gray-100 text-gray-800";
    ...
    if (!supplier) {
        return (
            <section className="w-full min-h-screen flex flex-col justify-center items-center gap-4">
                <p className="text-red-500 text-lg">Не вдалося завантажити дані постачальника.</p>
                <button
        ...
        <section className="p-10 w-full bg-gray-100 min-h-screen flex justify-center">
            <div className="w-full max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-md">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start mb-6 pb-4 border-b">
                    ...
                    {/* Статус */}
                    <span className={`mt-2 sm:mt-0 px-3 py-1 text-xs font-medium rounded-full ${getStatusStyle(supplier.status)}`}>

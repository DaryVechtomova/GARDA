...
const StatCard = ({ title, value, bgColor = 'bg-blue-500', icon }) => {
    // Якщо bgColor містить клас для кольору тексту (наприклад, text-gray-900 для жовтої картки),
    // він перекриє 'text-white' на батьківському div.
    return (
        <div
            className={`
...
const DashboardPage = () => {
    ...
    const [loadingStats, setLoadingStats] = useState(true); // Перейменовано для ясності
    ...
    useEffect(() => {
        ...
        const fetchDashboardStats = async () => {
            setLoadingStats(true); // Встановлюємо завантаження для основної статистики
            try {
                const response = await axios.get(`${API_URL}/api/admin-stats/dashboard`);
                if (response.data.success) {
                    setStats(response.data.data);
                } else {
                    toast.error(response.data.message || "Помилка завантаження основної статистики");
                }
            } catch (err) {
                ...
                setLoadingStats(false); // Завершуємо завантаження для основної статистики
            }
        };
        ...
        const fetchPopularProductsData = async () => {
            ...
            try {
                ...
                const popularResponse = await axios.get(`${API_URL}/api/admin-stats/popular-products?limit=5&days=30`);
                if (popularResponse.data.success) {
                    setPopularProducts(popularResponse.data.data);
                } else {
                    toast.error(popularResponse.data.message || "Помилка завантаження популярних товарів");
                }
            } catch (err) {
    ...
    if (loadingStats || loadingPopular) {
        return <div className="p-10 text-center text-xl text-gray-600">Завантаження даних...</div>;
    }

    // Умова помилки: показуємо, якщо основна статистика не завантажилася (популярні товари - другорядні)
    if (!stats) {
        return <div className="p-10 text-center text-red-600 bg-red-100 border border-red-400 rounded-md">Не вдалося завантажити основні дані статистики. Будь ласка, спробуйте оновити сторінку.</div>;
    }
    ...
    const getSafeStatValue = (value, isCurrency = false) => {
        if (typeof value === 'number') {
            return isCurrency ? value.toLocaleString('uk-UA', { style: 'currency', currency: 'UAH' }) : value;
        }
                    ...
                    {popularProducts.length > 0 ? (
                        <ul className="space-y-3">
                            {popularProducts.map((product, index) => (
                                <li key={product._id || index} className="flex items-center justify-between p-3 bg-gray-100 rounded-lg hover:bg-gray-100 transition-colors group">
                                    {/* Обгортка для посилання */}
                                    <NavLink
                                        to={`/admin_panel/product/details/${product._id}`}
                                        className="flex items-center overflow-hidden flex-grow mr-4" // flex-grow щоб зайняти доступне місце
                        ...
                        !loadingPopular && <p className="text-gray-500">Немає даних про популярні товари за вказаний період.</p>
                    )}
                    {loadingPopular && <p className="text-gray-500">Завантаження популярних товарів...</p>}
                </div>
            </div>
        </section>
    );

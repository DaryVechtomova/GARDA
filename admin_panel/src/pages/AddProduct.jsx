    ...
    const onSubmitHandler = async (event) => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                toast.error(response.data.message || "Не вдалося додати товар (помилка сервера).");
            }
        } catch (error) {
                        ...
                        {sizes.map((sizeItem, index) => (
                            <div key={index} className="flex flex-wrap items-center gap-x-3 gap-y-2 p-2 rounded border border-gray-200">
                                ...
                                <div className="flex flex-col gap-y-1 flex-grow sm:flex-grow-0 sm:w-40">
                                    <label htmlFor={`size-${index}`} className='text-xs font-medium text-gray-500'>Розмір</label>
                                    <select
                                        id={`size-${index}`}
                                ...
                                <div className="flex flex-col gap-y-1 flex-grow sm:flex-grow-0 sm:w-28">
                                    <label htmlFor={`quantity-${index}`} className='text-xs font-medium text-gray-500'>Кількість</label>
                                    <input
                                        id={`quantity-${index}`}
                                        type="number"
                                        name="quantity"
                                ...
                                <button
                                    ...
                                    className={`ml-auto mt-4 sm:mt-0 self-end px-2 py-2 bg-[#99120d] text-white rounded-md ${sizes.length > 1 ? 'hover:[#7a0e0a] cursor-pointer' : 'opacity-50 cursor-not-allowed'} transition-all`}
                                >
                                    &times;
                                </button>
                            </div>

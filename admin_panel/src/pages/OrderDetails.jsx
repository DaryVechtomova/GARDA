...
const OrderDetails = () => {
  ...
  const getStatusStyle = (status) => {
    switch (status) {
      ...
      case "Передано в службу доставки":
        return "bg-purple-100 text-purple-800";
      case "Чекає на отримання":
  ...
  const fetchOrderDetails = async () => {
    ...
    try {
      ...
      if (response.data.success && response.data.data) {
        ...
        if (
          ["Нове замовлення", "В обробці"].includes(response.data.data.status)
        ) {
          fetchProductStocks(response.data.data.items);
        }
      } else {
        toast.error(
          response.data.message || "Помилка завантаження даних замовлення"
        );
    ...
    const stockPromises = items.map(async (item) => {
      ...
      try {
        const productResponse = await axios.get(
          `${url}/api/product/details/${item.productId}`
        );
        ...
        console.error(
          `Помилка отримання залишків для ${item.productId} (${item.size}):`,
          error
        );
        return { productId: item.productId, size: item.size, quantity: 0 };
      }
  ...
  const updateOrderStatus = async () => {
    ...
    try {
      const response = await axios.put(
        `${url}/api/order/update-status/${order._id}`,
        {
          status: nextStatus,
        }
      );
      ...
      if (response.data.success) {
        ...
        setOrder((prev) => ({
          ...prev,
          status: nextStatus,
          editHistory: response.data.data?.editHistory || prev.editHistory,
        }));
    ...
    if (edit.type === "status_change") {
      return (
        <div>
          <p>
            Статус змінено з "
            <span className="font-medium">{edit.oldStatus}</span>" на "
    ...
    return (
      <>
        {edit.changes?.items?.length > 0 && (
          <ul className="list-disc pl-5 mt-1">
            {edit.changes.items.map((change, idx) => {
              ...
              return (
                <li key={idx}>
                  <span className="font-medium">{actionText}</span> товар:{" "}
                  {change.name || "Невідомий товар"}
        ...
        {edit.changes?.amountChanged && (
          <p className="mt-1">
            Сума змінена з {edit.changes.oldAmount.toFixed(2)} грн на{" "}
            {edit.changes.newAmount.toFixed(2)} грн
          </p>
        )}
      </>
    );
  ...
  const renderAmountChanges = (changes) => {
    ...
    return (
      <p className="mt-2">
        Сума змінена з {changes.oldAmount.toFixed(2)} грн на{" "}
        {changes.newAmount.toFixed(2)} грн
      </p>
    );
  };
  ...
  const canCancelOrder = ["Нове замовлення", "В обробці"].includes(
    order.status
  );
      ...
      <div className="w-full max-w-6xl mx-auto bg-white p-6 rounded-lg shadow-md print:shadow-none print:rounded-none print:p-4">
          ...
          mb-2
            ...
            <div className="mt-2 print:hidden">
              ...
              {order.status === "Скасовано" && order.cancellationReason && (
                <p className="text-xs text-[#99120d] mt-1 italic">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6 print:grid-cols-2 print:gap-4 print:mb-4">
          ...
          <div className="p-4 rounded-md border">
            ...
            <p className="text-sm mb-1">
              <span className="font-medium text-gray-700">Спосіб:</span>{" "}
            ...
            <div className="text-sm">
              ...
              <span className="text-gray-500">
                ...
                {order.deliveryMethod === "Нова Пошта" &&
                  `${order.deliveryDetails.region} область, м. ${order.deliveryDetails.city}, Відділення №${order.deliveryDetails.departmentNumber}`}
                {order.deliveryMethod === "Укрпошта" &&
                  `${order.deliveryDetails.region} область, м. ${
                    order.deliveryDetails.city
                  }, ${order.deliveryDetails.street} ${
                    order.deliveryDetails.houseNumber
                  }${
                    order.deliveryDetails.apartment
                      ? ", кв. " + order.deliveryDetails.apartment
                      : ""
                  }, поштовий індекс: ${order.deliveryDetails.postalCode}`}
                {order.deliveryMethod === "Самовивіз" &&
                  `м. ${order.deliveryDetails.city}`}
              </span>
            </div>
            {order.paymentMethod && (
              <div>
                <p className="text-sm mb-1">
                  ...
                  {/* Додай інші варіанти, якщо вони є */}
                  {
                    order.paymentMethod !== "payOnDelivery" &&
                      ...
                      order.paymentMethod /* Якщо невідомий метод, показуємо як є */
                  }
                </p>
              </div>
            )}
        ...
        <div className="mb-6 print:mb-4 overflow-x-auto">
          <h5 className="text-base font-semibold text-black mb-2 print:hidden">
            Склад замовлення
          </h5>
          <table className="w-full min-w-[600px] border-collapse text-sm">
            <thead className="bg-gray-100 print:hidden">
              <tr>
                <th className="p-2 border text-left font-semibold text-gray-600 w-16">
                  Фото
                </th>
                <th className="p-2 border text-left font-semibold text-gray-600">
                  Назва товару
                </th>
                <th className="p-2 border text-center font-semibold text-gray-600 w-20">
                  Розмір
                </th>
                <th className="p-2 border text-right font-semibold text-gray-600 w-24">
                  Ціна/шт.
                </th>
                <th className="p-2 border text-center font-semibold text-gray-600 w-16">
                  К-ть
                </th>
                <th className="p-2 border text-right font-semibold text-gray-600 w-28">
                  Сума
                </th>
              </tr>
            </thead>
              ...
              {order.items
                ...
                .map((item, index) => {
                  ...
                  const itemDiscountAmount =
                    item.discount > 0 ? (itemTotal * item.discount) / 100 : 0;
                  ...
                  return (
                    <tr
                      ...
                      className={`border-b ${
                        hasStockIssue ? "bg-red-50 print:bg-transparent" : ""
                      ...
                      <td className="p-2 border align-top print:p-1 print:border-0">
                        ...
                        {hasStockIssue && canEditOrder && (
                          <div className="text-xs text-[#99120d] font-medium mt-1 flex items-center gap-1 no-print">
                            <FaExclamationTriangle /> Недостатньо на складі! (Є:{" "}
                            {currentStock})
                          </div>
                        )}
                      </td>
                      <td className="p-2 border text-center align-top print:p-1 print:border-0">
                        {item.size}
                      </td>
                      <td className="p-2 border text-right align-top print:p-1 print:border-0">
                        {item.price.toFixed(2)} грн
                        {item.discount > 0 && (
                          <div className="text-xs text-[#99120d]">
                            (-{item.discount}%)
                          </div>
                        )}
                      </td>
          ...
          <div className="w-full max-w-xs text-sm">
            <div className="flex justify-between py-1">
              ...
              <span className="text-gray-800">
                {calculateTotalWithoutDiscount(order.items).toFixed(2)} грн
              </span>
            </div>
            {totalDiscount > 0 && (
              <div className="flex justify-between py-1 border-b">
                <span className="text-gray-600">Знижка:</span>
                <span className="text-[#99120d]">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 p-4 no-print">
          <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-md">
            <h3 className="text-lg font-semibold mb-4 text-gray-800">
              Скасування замовлення № {order.orderNumber}
            </h3>
            <div className="mb-4">
              <label
              ...
              >
                Причина скасування <span className="text-[#99120d]">*</span>
              </label>
              <select
            ...
            {cancelReason.includes("Інша причина") && (
              <div className="mb-4">
                <label
                  ...
                  className="block text-sm font-medium text-gray-600 mb-1"
                >
                  Деталі (обов'язково) <span className="text-[#99120d]">*</span>
                </label>
                <textarea
                  id="cancelComment"
                  value={cancelComment}

...
function List() {
  ...
  const removeProduct = async (productId) => {
    try {
      const response = await axios.post(`${url}/api/product/remove-product`, {
        id: productId,
      });
      if (response.data.success) {
  ...
  const removeDiscount = async (productId) => {
    try {
      const response = await axios.delete(
        `${url}/api/product/discount/remove/${productId}`
      );
      if (response.data.success) {
        toast.success(response.data.message);
        fetchList();
  ...
  const editDiscount = async (productId, discount) => {
    try {
      const response = await axios.put(
        `${url}/api/product/discount/edit/${productId}`,
        { discount }
      );
      if (response.data.success) {
        toast.success(response.data.message);
  ...
  const sortedAndFilteredProducts = searchProducts(
    filterProducts(sortProducts(list))
  );
        ...
        <div className="overflow-auto max-h-[calc(100vh-236px)]">
          <table className="w-full border-collapse border border-gray-200">
            <thead className="bg-gray-100 sticky top-0">
              {" "}
              {/* Залишаємо заголовок таблиці видимим при прокрутці */}
              <tr>
                <th className="p-3 border">Продукт</th>
                ...
                <tr key={product._id}>
                  ...
                  <td className="p-3 border text-center">
                    ...
                    <div className="flex gap-2 justify-center mt-2">
                      <button
                        onClick={() =>
                          openEditDiscountModal(product._id, product.discount)
                        }
                  ...
                  <td className="p-3 border justify-center items-center">
                    <NavLink
                      to={`/admin_panel/edit-product/${product._id}`}
                      className="text-blue-500 hover:text-blue-700 flex justify-center"
                    >
                      <TbEdit size={20} />
                    </NavLink>

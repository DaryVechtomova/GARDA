...
const AddInvoice = () => {
  ...
  const addProductToList = () => {
    ...
    if (isNaN(quantity) || quantity <= 0) {
      toast.error("Кількість товару має бути позитивним числом");
      return;
    }
    if (pricePerUnit <= 0) {
      toast.warn(
        "Ціна за одиницю 0 або не розрахована. Товар буде додано з ціною 0."
      );
    }
    ...
    if (!productExists) {
      toast.error("Обраний товар не знайдено в системі.");
      return;
    }
  ...
  const addAllSizesToList = () => {
    // Перевіряємо введені дані
    if (!selectedProductOption || !selectedProductOption.value) {
      toast.error("Будь ласка, спочатку оберіть товар");
      return;
    }
    const quantityNum = parseInt(quantityForAllSizes);
    if (isNaN(quantityNum) || quantityNum <= 0) {
      toast.error(
        "Будь ласка, введіть коректну кількість (> 0) для всіх розмірів"
      );
      return;
    }
    if (pricePerUnit <= 0) {
      toast.warn(
        "Ціна за одиницю 0 або не розрахована. Товари будуть додані з ціною 0."
      );
    }
    ...
    toast.success(
      `Додано ${allSizes.length} розмір(и) товару "${
        selectedProductOption.label.split(" (ID:")[0]
      }"`
    );
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      const errorMessage =
        ...
        error.message ||
        "Не вдалося додати накладну (невідома помилка)";
      ...
      >
        ...
        <fieldset className="border border-gray-300 p-4 rounded-md">
          <legend className="text-base font-medium px-2 text-gray-900">
            Додати товар <span className="text-red-500">*</span>
          </legend>
          {/* Ряд 1: Вибір товару */}
          <div className="mb-4">
            <label
            ...
            <Select
              ...
              noOptionsMessage={() =>
                filteredProducts.length === 0 && data.supplier
                  ? "Немає доступних товарів для цього постачальника"
            ...
            <div className="flex items-end gap-2 border-l border-gray-300 pl-4 ml-auto">
              ...
              <div className="flex flex-col gap-y-1">
                ...
                <input
                  ...
                  disabled={
                    !selectedProductOption || getAvailableSizes().length === 0
                  }
                />
              </div>
              ...
              <button
                ...
                disabled={
                  !selectedProductOption ||
                  !quantityForAllSizes ||
                  parseInt(quantityForAllSizes) <= 0 ||
        ...
        {selectedProducts.length > 0 && ( // Показуємо блок, тільки якщо є товари
          <div className="flex flex-col gap-y-2 mt-4">
            ...
            <div className="border border-gray-300 rounded-md max-h-60 overflow-y-auto">
              {" "}
              {/* Додано обгортку для тіні/скролу */}
                    ...
                    >
                      <span className="flex-1 mr-2">
                        <span className="font-medium text-gray-800">
                          {product?.name || `Товар ID: ${item.product}`}
                        </span>
                        <span className="text-gray-600">
                          {" "}
                          (Розмір: {item.size}, К-ть: {item.quantity}, Ціна:{" "}
                          {displayPrice.toFixed(2)} грн)
                        </span>
                      </span>
                      <button
                        type="button"
                        title="Видалити товар зі списку"
                        className="text-[#99120d] hover:text-[#7a0e0a] font-medium text-xs px-2 py-1 rounded hover:bg-red-50 transition duration-150 ease-in-out"

...
const ProductComments = ({ productId }) => {
  ...
  const formatDate = (dateString) => {
    ...
    try {
      ...
      if (dateString === "13.04.2025") return "13.04.2025";
    ...
    const fetchComments = async () => {
      ...
      try {
        const response = await fetch(
          `${url}/api/review/reviews-user/${productId}`
        );
        if (!response.ok) {
        ...
        if (data.success && Array.isArray(data.data)) {
          ...
          console.warn(
            "API returned success but data is not an array:",
            data.data
          );
  ...
  const handleCommentSubmit = async (event) => {
    ...
    if (!token) {
      toast.error(
        "Ви не авторизовані. Будь ласка, увійдіть, щоб залишити коментар."
      );
      return;
    }
    ...
    try {
      ...
      if (response.data.success) {
        toast.success(response.data.message || "Відгук додано!");

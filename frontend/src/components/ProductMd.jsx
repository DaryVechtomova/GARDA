...
const ProductMd = ({ product }) => {
  ...
  const wishlistItems = contextValue?.wishlistItems;
  ...
  const [isCurrentProductFavorited, setIsCurrentProductFavorited] =
    useState(false);
  ...
  const numberOfVisibleSmallImages =
    totalImages > 1 ? Math.min(2, totalImages - 1) : 0;

  let sliderTrackHeight = 290;
  let sliderThumbHeight = 44;
  let sliderTrackElementTop = 97;
  let sliderTrackOffsetWithinControls = sliderTrackElementTop - 40;
  if (typeof window !== "undefined") {
    const screenWidth = window.innerWidth;
    if (screenWidth >= 320 && screenWidth <= 450) {
      sliderTrackHeight = 161;
      sliderThumbHeight = 24;
      sliderTrackElementTop = 62;
      sliderTrackOffsetWithinControls = sliderTrackElementTop;
    } else if (screenWidth >= 451 && screenWidth <= 999) {
      sliderTrackHeight = 290;
      sliderThumbHeight = 44;
      sliderTrackElementTop = 188;
      sliderTrackOffsetWithinControls = 188 - 131;
    } else if (screenWidth >= 1000 && screenWidth <= 1439) {
      sliderTrackHeight = 249;
      sliderThumbHeight = 38;
      sliderTrackElementTop = 102;
      sliderTrackOffsetWithinControls = 102 - 53;
    } else {
      sliderTrackHeight = 290;
      sliderThumbHeight = 44;
      sliderTrackElementTop = 97;
      sliderTrackOffsetWithinControls = 97 - 40;
    }
  }
  ...
  useEffect(() => {
    ...
    const currentEndVisibleIndex =
      visibleStartIndex + numberOfVisibleSmallImages - 1;
    ...
    newVisibleStartIndex = Math.max(
      0,
      Math.min(newVisibleStartIndex, maxStartIndex)
    );
  ...
  const handleToggleFavorite = (event) => {
    ...
    if (toggleWishlist && product && product._id) {
      ...
      console.error(
        "Неможливо оновити статус улюбленого: функція toggleWishlist або ID товару відсутні."
      );
    }
  };
  ...
  const handleAddToCartClick = () => {
    ...
    if (!addToCart) {
      // addToCart береться з useContext(ShopContext)
    ...
    const sizeToAdd =
      product.sizes && product.sizes.length > 0 ? selectedSize : "N/A";
  ...
  const handleAddToCartWrapper = (event) => {
    // Перейменував, щоб уникнути конфлікту імен, якщо addToCart з контексту використовується напряму
    event.stopPropagation();
    if (!product || !product._id) {
      console.error("Product ID не визначено");
      return;
    ...
    if (addToCart) {
      // Перевірка, чи функція існує в контексті
  ...
  const handleIncrementCartItem = (event) => {
    ...
    if (addToCart && product && product._id) {
      ...
      const sizeOfItemInCart = currentCartItem
        ? currentCartItem.size
        : selectedSize; // Беремо розмір з кошика або обраний
  ...
  const currentItemDataInCart =
    product && product._id ? cartItems?.[product._id] : null;
  const currentQuantityInCart = currentItemDataInCart
    ? currentItemDataInCart.quantity
    : 0;

  const sizeListText =
    product.sizes?.map((sizeObj) => sizeObj.size).join(" ") || "Немає";
  const thumbTopWithinTrack =
    totalImages > 1
      ? (sliderTrackHeight - sliderThumbHeight) *
        (sliderIndex / (totalImages - 1))
      : 0;
  ...
  if (totalImages > 1 && numberOfVisibleSmallImages > 0) {
    ...
    if (availableIndexes.length <= numberOfVisibleSmallImages) {
      ...
      for (let i = 0; i < numberOfVisibleSmallImages; i++) {
        ...
        if (attempts < totalImages) {
          // Якщо знайшли не поточний індекс
      ...
      <section className="product-container ProductMd-section">
        ...
        <div className="product-images-container">
          ...
          {totalImages > 1 && (
            <div className="slider-controls">
              <button
                className="slider-arrow up-arrow"
              ...
              <div className="slider-track">
                <div
                  ...
                  style={{ top: `${thumbTopWithinTrack}px` }}
                ></div>
              </div>
              <button
                className="slider-arrow down-arrow"
              ...
              {imagesToDisplayInSmallSlider.map((imgData) => {
                ...
                return (
                  <img
                    ...
                    className={
                      imgData.originalIndex === sliderIndex
                        ? "semi-active-thumb"
                        : ""
                    }
          ...
          <div className="main-image" onClick={handleMainImageClick}>
            ...
            {currentImageFilename ? (
              <img
                ...
                onError={(e) => {
                  console.error(
                    `Помилка завантаження головної картинки: ${url}/images/${currentImageFilename}`
                  );
        ...
        <div className="product-info">
          ...
          {product.sizes && product.sizes.length > 0 && (
            <div className="my-4">
              {" "}
              {/* головний контейнер для цього блоку */}
        ...
        >
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              closeImageModal();
            }} // Зупиняємо спливання, щоб не закривалось при кліку на кнопку
            aria-label="Закрити"
          >
            <FaTimes />
          </button>

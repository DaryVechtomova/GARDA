...
const getStatusIcon = (status) => {
  switch (status) {
    case "Нове замовлення":
      return <FaHourglassHalf className="mr-1 inline" />;
    case "Оплачено":
      return <FaCheckCircle className="mr-1 inline text-cyan-700" />;
    case "В обробці":
      return <FaBoxOpen className="mr-1 inline" />;
    case "Передано в службу доставки":
      return <FaShippingFast className="mr-1 inline" />;
    case "Чекає на отримання":
      return <FaShippingFast className="mr-1 inline text-orange-700" />;
    case "Доставлено":
      return <FaCheckCircle className="mr-1 inline text-green-700" />;
    case "Скасовано":
      return <FaTimesCircle className="mr-1 inline text-red-700" />;
    case "Оплата не вдалася":
      return <FaTimesCircle className="mr-1 inline text-red-800" />;
    default:
      return null;
  }
};
    ...
    const fetchOrders = async () => {
      ...
      try {
        ...
        if (response.data.success) {
          ...
          toast.error(
            response.data.message || "Не вдалося завантажити замовлення."
          );
        }
      } catch (error) {
      ...
      <section className='min-h-screen pt-28 pb-16 font-["Literata"]'>
        <div className="max-w-[1440px] mx-auto px-4 md:px-8 xl:px-20 text-center py-20">
          <p className="text-xl text-gray-700 mb-6">
            ...
            <Link
              to="/login"
              className="text-[#54A5D9] hover:underline font-semibold"
            >
              увійдіть
            </Link>
            , щоб переглянути історію замовлень.
          </p>
        </div>
      </section>
    );
    ...
    <section className='min-h-screen pt-28 pb-16 font-["Literata"]'>
      <div className="max-w-[1440px] mx-auto px-4 md:px-8 xl:px-20">
        ...
        <div className="space-y-8">
          {" "}
          {/* Збільшив відступ між картками */}
          {orders.map((order) => (
            <div
              key={order._id}
              className="bg-white shadow-xl rounded-[30px] p-4 sm:p-6 md:p-8 border border-gray-200/50"
            >
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 pb-4 border-b border-gray-300/70">
                ...
                <div
                  className={`mt-3 sm:mt-0 px-3 py-1.5 text-xs sm:text-sm font-semibold rounded-full ${getStatusStyle(
                    order.status
                  )} flex items-center`}
                >
                ...
                {order.items
                  ...
                  .map((item) => (
                    <div
                      ...
                      className="flex items-start sm:items-center py-3 border-b border-gray-200/60 last:border-b-0"
                    >
                      <img
                        src={
                          item.image
                      ...
                      <div className="flex-grow">
                        <p className="text-sm sm:text-base font-semibold text-gray-900 leading-tight mb-1">
                          {item.name}
                        </p>
                        <p className="text-xs sm:text-sm text-gray-600">
                          Розмір: {item.size === "N/A" ? "Стандарт" : item.size}
                        </p>
                        <p className="text-xs sm:text-sm text-gray-600">

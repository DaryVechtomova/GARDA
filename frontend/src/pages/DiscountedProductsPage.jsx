...
import Item from "../components/Item"; // Переконайтесь, що шлях до Item правильний
import { ShopContext } from "../context/ShopContext"; // Переконайтесь, що шлях до ShopContext правильний
...
const DiscountedProductsPage = () => {
    ...
    // Ефект для відстеження зміни ширини вікна (залишаємо як у SearchPage)
    useEffect(() => {
        const handleResize = () => {
    ...
    useEffect(() => {
        const fetchDiscountedProducts = async () => {
            ...
            try {
                const response = await fetch(`${url}/api/product/list-discounted-products`);
                ...
                if (data.success) {
                    ...
                    throw new Error(data.message || "Не вдалося завантажити товари зі знижкою.");
                }
            } catch (err) {
    ...
    return (
        ...
        >
            ...
            {isLoading ? (
                ...
                <div className="flex-grow flex items-center justify-center">
                    <p className="text-center text-lg">Наразі немає акційних товарів.</p> {/* Змінено повідомлення */}
                </div>
            )}
        </section>
    );

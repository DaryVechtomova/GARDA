...
import { useNavigate } from 'react-router-dom'; // Для перенаправлення, якщо користувач не авторизований
import Item from "../components/Item"; // Переконайся, що шлях правильний
...
const  Favorites = () => {
    ...
    useEffect(() => {
        ...
        if (all_products && wishlistItems) {
            ...
            const wishlistedIds = Object.keys(wishlistItems).filter(id => wishlistItems[id] > 0);

            // Фільтруємо all_products, залишаючи тільки ті, що є в wishlistedIds
            const filteredProducts = all_products.filter(product =>
                wishlistedIds.includes(product._id) // Переконайся, що ID товару називається _id
            );
        ...
        return (
            ...
            >
                ...
                <p className="text-center text-lg mb-4">
                    Будь ласка, увійдіть до свого облікового запису, щоб переглянути обрані товари.
                </p>
                <button

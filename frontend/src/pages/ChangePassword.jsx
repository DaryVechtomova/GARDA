...
import { FaSave, FaArrowLeft, FaSpinner, FaEye, FaEyeSlash } from 'react-icons/fa'; // Оновив іконки
...
const ChangePasswordPage = () => {
    ...
    const validateForm = () => {
        ...
        } else if (passwords.newPassword && passwords.confirmPassword !== passwords.newPassword) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                navigate('/profile'); // Повертаємось до профілю
            } else {
                // Якщо бекенд повертає помилку про старий пароль, показуємо її біля поля
                if (response.data.message && response.data.message.toLowerCase().includes("старий пароль")) {
                    setErrors(prev => ({ ...prev, oldPassword: response.data.message }));
                } else {
            ...
            if (error.response?.data?.message && error.response.data.message.toLowerCase().includes("старий пароль")) {
                setErrors(prev => ({ ...prev, oldPassword: error.response.data.message }));
            } else {
        ...
        "
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex flex-col gap-y-1">
                        <label htmlFor="oldPassword" className='text-sm font-medium text-gray-600'>Старий пароль <span className="text-red-500">*</span></label>
                        <div className='relative'>
                            <input
                                ...
                                className={`border rounded-md py-1.5 pl-3 pr-10 h-[38px] w-full outline-none focus:ring-1 focus:ring-offset-1 transition duration-150 ease-in-out ${errors.oldPassword ? 'border-red-500 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 focus:ring-blue-500 focus:border-blue-500'}`}
                            ...
                            <button
                                ...
                                aria-label={showPassword.old ? "Сховати пароль" : "Показати пароль"}
                            >
                                {/* Умова для іконки */}
                                {showPassword.old ? <FaEye size={16} /> : <FaEyeSlash size={16} />}
                            </button>
                        </div>
                    ...
                    {/* Новий пароль */}
                    <div className="flex flex-col gap-y-1">
                        <label htmlFor="newPassword" className='text-sm font-medium text-gray-600'>Новий пароль <span className="text-red-500">*</span></label>
                        <div className='relative'>
                            <input
                                id="newPassword"
                                name="newPassword"
                                ...
                                onChange={handleChange}
                                className={`border rounded-md py-1.5 pl-3 pr-10 h-[38px] w-full outline-none focus:ring-1 focus:ring-offset-1 transition duration-150 ease-in-out ${errors.newPassword ? 'border-red-500 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 focus:ring-blue-500 focus:border-blue-500'}`}
                            ...
                            <button
                                ...
                                className='absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-500 hover:text-gray-700 focus:outline-none'
                                aria-label={showPassword.new ? "Сховати пароль" : "Показати пароль"}
                            >
                                {/* Умова для іконки */}
                                {showPassword.new ? <FaEye size={16} /> : <FaEyeSlash size={16} />}
                            </button>
                        </div>
                    ...
                    <div className="flex flex-col gap-y-1">
                        <label htmlFor="confirmPassword" className='text-sm font-medium text-gray-600'>Підтвердіть новий пароль <span className="text-red-500">*</span></label>
                        <div className='relative'>
                            <input
                                id="confirmPassword"
                                ...
                                onChange={handleChange}
                                className={`border rounded-md py-1.5 pl-3 pr-10 h-[38px] w-full outline-none focus:ring-1 focus:ring-offset-1 transition duration-150 ease-in-out ${errors.confirmPassword ? 'border-red-500 focus:ring-red-500 focus:border-red-500' : 'border-gray-300 focus:ring-blue-500 focus:border-blue-500'}`}
                                autoComplete="new-password"
                            />
                            <button
                                type="button"
                                ...
                                className='absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-500 hover:text-gray-700 focus:outline-none'
                                aria-label={showPassword.confirm ? "Сховати пароль" : "Показати пароль"}
                            >
                                {/* Умова для іконки */}
                                {showPassword.confirm ? <FaEye size={16} /> : <FaEyeSlash size={16} />}
                            </button>
                        </div>

...
const ShopContextProvider = (props) => {
  ...
  const loadUserProfile = async (tokenToUse) => {
    ...
    try {
      ...
      if (response.data.success && response.data.userData) {
        ...
        console.log(
          "Профіль користувача завантажено в контекст:",
          response.data.userData
        );
      } else {
        console.error(
          "Не вдалося завантажити профіль в контекст:",
  ...
  const addToCart = async (itemId, selectedSize) => {
    if (!selectedSize || selectedSize === "N/A") {
      ...
      if (productInfo && productInfo.sizes && productInfo.sizes.length > 0) {
        ...
        console.warn(
          "Спроба додати товар без розміру, хоча він потрібен:",
          itemId
        );
        return;
      }
  ...
  const removeFromCart = async (itemId, itemSize) => {
    // Тепер removeFromCart має знати РОЗМІР
    if (!itemSize) {
      ...
      if (cartItem && cartItem.quantity > 0) {
        ...
        if (productInfo) {
          const priceAfterDiscount = Math.round(
            productInfo.price * (1 - (productInfo.discount || 0) / 100)
          );
  ...
  const loadWishlistData = async (token) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        console.error(
          "Помилка отримання Обраного (з бекенду):",
          response.data.message
        );
  ...
  const addToWishlist = async (itemId) => {
    ...
    try {
      ...
      if (!response.data.success) {
        console.error(
          "Помилка додавання в обране (з бекенду):",
          response.data.message
        );
      ...
      console.error(
        "Помилка ЗАПИТУ додавання в обране:",
  ...
  const removeFromWishlist = async (itemId) => {
    ...
    try {
      ...
      if (!response.data.success) {
        console.error(
          "Помилка видалення з обраного (з бекенду):",
          response.data.message
        );
  ...
  const toggleWishlist = (itemId) => {
    if (wishlistItems[itemId] && wishlistItems[itemId] > 0) {
      // Перевіряємо чи є і кількість > 0 (згідно з бекендом)
  ...
  const loadCartData = async (tokenToUse) => {
    // <--- (A) Тепер приймає аргумент
    console.log("loadCartData викликана з токеном:", tokenToUse); // <--- (B) Виводимо токен, який прийшов
    if (!tokenToUse) {
      // <--- (C) Перевіряємо токен, який прийшов як аргумент
      console.warn(
        "loadCartData: Токен не передано або він порожній — неможливо завантажити кошик."
      );
    ...
    try {
      ...
      if (response.data.success) {
        console.log(
          "loadCartData: Отримані дані кошика:",
          response.data.cartData
        );
        setCartItems(response.data.cartData || {}); // Додано || {} для безпеки
      } else {
        console.error(
          "loadCartData: Помилка при отриманні кошика (з бекенду):",
          response.data.message
        );
        setCartItems({});
      }
      ...
      console.error(
        "loadCartData: Запит на кошик не вдався:",
        error.response ? error.response.data : error.message
      );
      setCartItems({});
    }
